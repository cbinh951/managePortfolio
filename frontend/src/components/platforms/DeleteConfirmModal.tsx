...
export default function DeleteConfirmModal({
    ...
    const handleDelete = async () => {
        ...
        try {
            setLoading(true);
            setError('');
            await onDelete(platform.platform_id);
            onSuccess();
            onClose();
        } catch (err) {
            ...
            <div className="relative bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-full max-w-md mx-4">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-700">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold text-white">Delete Platform</h2>
                    </div>
                    <button
                        onClick={handleClose}
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                        <p className="text-sm text-amber-400">
                            <strong>Warning:</strong> You cannot delete a platform that is currently in use by portfolios or cash accounts.
                        </p>
                    </div>
