...
export function createAssetAnalyticsRoutes(csvService: CsvService): Router {
    const router = express.Router();
    const assetAnalyticsService = new AssetAnalyticsService(csvService);
    ...
    router.get('/', async (req: Request, res: Response) => {
        try {
            ...
            const metrics = await assetAnalyticsService.getAssetTypeMetrics(assetType);
